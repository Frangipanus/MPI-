...
pthread_mutex_t actions = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t remise_bool = PTHREAD_MUTEX_INITIALIZER;
...
void* lutins(void* v_untyped){
    ...
    while(v->nb_action_ind < v->taille){
        if((v->nb_action_ind <= (v->phase[0]/2)) && v->phase[0] % 2 == (i % 2)){
            ...
            pthread_mutex_lock(&actions);
            v->nb_action[0] = v->nb_action[0]+1;
            pthread_mutex_unlock(&actions);
