...
let find_index tab elem = 
  let length = Array.length tab in
  let i = ref 0 in 
  let found = ref false in
  while not(!found) && (!i < length) do
    match tab.(!i) with 
    |Some (f) -> (found := alpha_equiv f elem; if(not(!found)) then i := !i + 1)
    |None -> i := !i + 1
  done;
...
let same_hyp2 c1 c2 = 
  ...
  if (List.length c1 != List.length c2) then false 
  else begin  let length = List.length c1 in
  let pos1 = ref 0 in 
  let pos2 = ref 0 in 
  let t1 = Array.make (length) (0) in 
  let t2 = Array.make (length) (0) in
  let r1 = Array.make (length) (None) in 
  let r2 = Array.make (length) (None) in
  List.iter (fun x -> 
    match find_index r1 x with 
      |None -> (t1.(!pos1) <- 1; r1.(!pos1) <- (Some(x)) ;pos1 := !pos1 + 1; )
      |Some(i) -> (t1.(i) <- t1.(i)+1 ) 
      ) c1;
  List.iter (fun x -> 
    match find_index r2 x with 
      |None -> (t2.(!pos2) <-1; r2.(!pos2) <- (Some(x)) ;pos2 := !pos2 + 1; )
      |Some(i) -> (t2.(i) <- t2.(i)+1 ) 
      ) c1;
    
  let res = ref true in 
  List.iter (fun f -> let acc = (match  find_index r1 f with
                                  |Some(a)-> a
                                  |None -> failwith "gros probleme") in 
             match find_index r2 f with 
             |None -> res := false
             |Some(i) -> if(t1.(acc) != t2.(i)) then res:=false) c1; 
  !res
  end
